...
from discovery_utils.iou3d_nms import iou3d_nms_utils
...
def PCA_rectangle(cluster_ptc):
    components = sklearn.decomposition.PCA(n_components=2).fit(cluster_ptc).components_
...
def objs_nms(objs, use_score_rank=False, nms_threshold=0.1):
    ...
    overlaps_bev = iou3d_nms_utils.boxes_iou_bev(boxes.contiguous(), boxes.contiguous())
...
def objs_lidar_nms(annos_info, use_score_rank=False, nms_threshold=0.1):
    boxes = np.copy(annos_info['boxes_lidar'])
    boxes[:, 2] = 0  # zero out the z-axis for bev nms
    boxes = torch.from_numpy(boxes).float().cuda()
    ...
    for idx in order:
        ...
        mask[idx] = True
    annos_info_nmsed = {
        "boxes_lidar": annos_info['boxes_lidar'][mask],
        "name": annos_info['name'][mask],
        "metadata": annos_info['metadata']
    }
    return annos_info_nmsed
...
def is_within_fov(obj, calib, image_shape):
    ...
    return (
        uv[0] < image_shape[1]
        and uv[0] >= 0
        and uv[1] < image_shape[0]
        and uv[1] >= 0
        and center[2] > 0
    )

def obs_within_fov(annos_info, ref_to_cam, cam_intrinsic, img_shape):
    center = np.copy(annos_info['boxes_lidar'][:, :3])  # lidar
    center_rect = transform_points(center, ref_to_cam) 
    center_2d = np.dot(center_rect, cam_intrinsic.T)
    center_img = (center_2d[:, 0:2].T / center_2d[:, 2]).T  # (N, 2)
    center_rect_depth = center_rect[:, 2]
    val_flag_1 = np.logical_and(center_img[:, 0] >= 0, center_img[:, 0] < img_shape[0])
    val_flag_2 = np.logical_and(center_img[:, 1] >= 0, center_img[:, 1] < img_shape[1])
    val_flag_merge = np.logical_and(val_flag_1, val_flag_2)
    mask = np.logical_and(val_flag_merge, center_rect_depth >= 0)
    annos_info_fov = {
        "boxes_lidar": annos_info['boxes_lidar'][mask],
        "name": annos_info['name'][mask],
        "metadata": annos_info['metadata']
    }
    return annos_info_fov