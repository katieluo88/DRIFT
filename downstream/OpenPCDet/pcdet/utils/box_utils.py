...
def remove_points_in_boxes3d(points, boxes3d, return_mask=False):
    ...
    if return_mask:
        return points.numpy() if is_numpy else points, point_masks.sum(dim=0) == 0
    else:
...
def offset_box3d(boxes3d, extra_offset=(0, 0, 0)):
    """
    Args:
        boxes3d: [x, y, z, dx, dy, dz, heading], (x, y, z) is the box center
        extra_offset: [extra_x, extra_y, extra_z]
    ...
    large_boxes3d[:, 0:3] += boxes3d.new_tensor(extra_offset)[None, :]
    return large_boxes3d
