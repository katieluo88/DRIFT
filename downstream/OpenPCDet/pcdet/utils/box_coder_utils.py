...
class ResidualCoder(object):
    ...
    def decode_torch(self, box_encodings, anchors):
        """
        Args:dense_total_unsup_loss_reg
        Returns:
...
class PointResidualCoder(object):
    ...
    def decode_torch_soft(self, box_encodings, points, pred_classes=None, temperature=1.0):
        """
        Args:
            box_encodings: (N, 8 + C) [x, y, z, dx, dy, dz, cos, sin, ...]
            points: [x, y, z]
            pred_classes: (N, num_classes) [1, num_classes] as logits
        Returns:
        ...
        # assert pred_classes.max() <= self.mean_size.shape[0]
        # point_anchor_size = self.mean_size[pred_classes - 1]
        pred_classes = F.softmax(pred_classes / temperature, dim=-1)
        # sizes = [pred_classes[..., i] * size for i, size in enumerate(self.mean_size)]
        # tensor_mean = torch.tensor(self.mean_size).to(pred_classes.type).to(pred_classes.device)  # 4 x 3
        point_anchor_size = torch.einsum('nc,cd->nd', pred_classes, self.mean_size)

        # point_anchor_size = torch.sum(pred_classes * tensor_mean, dim=-1, keepdim=True)
        dxa, dya, dza = torch.split(point_anchor_size, 1, dim=-1)
        diagonal = torch.sqrt(dxa ** 2 + dya ** 2)
        xg = xt * diagonal + xa
        yg = yt * diagonal + ya
