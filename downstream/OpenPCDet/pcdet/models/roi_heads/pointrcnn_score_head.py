...
class PointRCNNScoreHead(RoIHeadTemplate):
    def __init__(self, input_channels, model_cfg, num_class=1, **kwargs):
        ...
        self.cls_layers = self.make_fc_layers(
            input_channels=channel_in, output_channels=1, fc_list=self.model_cfg.CLS_FC
        )
    ...
    def forward(self, batch_dict):
        ...
        if self.training:
            if self.model_cfg.LOSS_CONFIG.get('USE_UNSUPERVISED', False):
                ...
                # targets_dict = self.assign_targets_unsupervised(batch_dict, targets_dict)
            else:
        ...
        # rcnn_int_cls = self.int_cls_layers(shared_features).transpose(1, 2).contiguous().squeeze(dim=1)  # (B, num_cls)
        ...
        if not self.training:
            ...
            # batch_dict['batch_int_cls_preds'] = batch_int_cls_preds
            batch_dict['cls_preds_normalized'] = False
        else:
            targets_dict['rcnn_cls'] = rcnn_cls
            targets_dict['rcnn_reg'] = rcnn_reg
            # targets_dict['rcnn_int_cls'] = rcnn_int_cls
