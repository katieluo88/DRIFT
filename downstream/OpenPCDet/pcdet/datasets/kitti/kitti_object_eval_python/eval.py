...
def filter_preds(det_annos, gt_annos, threshold=0.5):
    """
    Assign class to each cluster ID. Can be truck, car, pedestrian, false positive (FP), or bicyclist
    
    Args:
        det_annos (List): List of detection annotations (and associated predicted cluster ID) from detector
        gt_annos (List): List of groundtruth detections
        threshold (float): Threshold to assign the gt annotation labels to (DEFAULT: 0.5)
        
    Returns:
        det_labels, gt_labels (Tuple): Detector cluster ID predictions, gt assignments
    """
    assert len(det_annos) == len(gt_annos)
    
    overlaps, parted_overlaps, total_dt_num, total_gt_num = calculate_iou_partly(gt_annos, det_annos, metric=1)
    
    det_preds = []
    gt_labels = []
    for i in range(len(det_annos)):
        overlap = overlaps[i]
        det_anno = det_annos[i]
        gt_anno = gt_annos[i]
        
        gt_labels_elem_mat = (np.where((overlap > threshold).T, gt_anno['name'], 'FP')).T
        
        for j in range(gt_labels_elem_mat.shape[1]):
            label = gt_labels_elem_mat[:, j][gt_labels_elem_mat[:, j] != 'FP']
        ...
        det_preds.extend([int(i) for i in list(det_anno['name'])])
    
    return det_preds, gt_labels

def clean_data(gt_anno, dt_anno, current_class, difficulty):
    CLASS_NAMES = ['car', 'pedestrian', 'cyclist', 'van', 'person_sitting', 'truck', 'dynamic']
    MIN_HEIGHT = [40, 25, 25]
    MAX_OCCLUSION = [0, 1, 2]
    MAX_TRUNCATION = [0.15, 0.3, 0.5]
    dc_bboxes, ignored_gt, ignored_dt = [], [], []
    # current_cls_name = str(current_class).lower()
...
def do_eval(gt_annos,
    ...
    # pickle.dump(PR_detail_dict, open('PR_detail_dict.pkl', 'wb'))
...
def get_range_eval_result(gt_annos,
    ...
    overlap_0_5 = np.array([[0.7, 0.5, 0.5, 0.7, 0.5, 0.5, 0.5],
                            ...
                            [0.5, 0.25, 0.25, 0.5, 0.25, 0.5, 0.5]])
    # overlap_0_7 = np.array([[[0.5] * len(current_classes)] * 3]).squeeze()
    # overlap_0_5 = np.array([[[0.5] * len(current_classes)] * 3]).squeeze()
    # print(overlap_0_7.shape)
    min_overlaps = np.stack([overlap_0_7, overlap_0_5], axis=0)  # [2, 3, 5]
    # min_overlaps = np.stack([overlap_0_9, overlap_0_7, overlap_0_5], axis=0)  # [2, 3, 5]
    class_to_name = {
        0: 'Car',
        1: 'Pedestrian',
        2: 'Cyclist',
    ...
    # class_to_name = {i: a for i, a in enumerate(current_classes)}
    name_to_class = {v: n for n, v in class_to_name.items()}
    if not isinstance(current_classes, (list, tuple)):
        current_classes = [current_classes]
    current_classes_int = []
    ...
    for range_s, range_e in range_pairs:
        ...
        for j, curcls in enumerate(current_classes):
            ...
            # ret_dict[f'{curcls_name}_3d_iou0.9/{range_s:02d}-{range_e:02d}_R40'] = mAP3d_R40[j, 0, 2]
            ...
            # ret_dict[f'{curcls_name}_bev_iou0.9/{range_s:02d}-{range_e:02d}_R40'] = mAPbev_R40[j, 0, 2]
    ...
    for j, curcls in enumerate(current_classes):
        ...
        threeD05 = [ret_dict[f'{curcls_name}_3d_iou0.5/{range_s:02d}-{range_e:02d}_R40']
                    for range_s, range_e in range_pairs]
        # bev09 = [ret_dict[f'{curcls_name}_bev_iou0.9/{range_s:02d}-{range_e:02d}_R40']
        #          for range_s, range_e in range_pairs]
        # threeD09 = [ret_dict[f'{curcls_name}_3d_iou0.9/{range_s:02d}-{range_e:02d}_R40']
        #             for range_s, range_e in range_pairs]
        ...
        #     "  ".join(
        #         [f"{range_s:02d}-{range_e:02d} " for range_s, range_e in range_pairs]) + "\n"
        # result += "BEV:  " + ", ".join([f"{x:6.3f}" for x in bev09]) + "\n"
        # result += "3D :  " + ", ".join([f"{x:6.3f}" for x in threeD09]) + "\n"
        # result += f"{curcls_name} IoU 0.7:\n"
        # result += ", ".join([f"{x:3.1f} / {y:3.1f}" for x,
        #                     y in zip(bev09, threeD09)]) + "\n"
        result += f"{curcls_name} IoU 0.7:\n"
        result += ", ".join([f"{x:3.1f} / {y:3.1f}" for x,
