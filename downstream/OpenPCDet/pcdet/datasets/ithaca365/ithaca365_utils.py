...
def fill_modest_train_infos(data_path, nusc, train_scenes, max_sweeps=1, THRESHOLD_NUM_POINTS=1000):
    train_nusc_infos = []

    ref_chan = 'LIDAR_TOP'  # The radar channel from which we track back n sweeps to aggregate the point cloud.
    chan = 'LIDAR_TOP'  # The reference channel of the current sample_rec that the point clouds are mapped to.

    for location in nusc.location:
        location_sample_datas = location['sample_data_dict']
        for scene_token in train_scenes:
            if scene_token not in location_sample_datas:
                print("Missing scene token in location:", scene_token, location['token'])
                continue

            sample_data_dict = location_sample_datas[scene_token]
            cam_data_token = sample_data_dict['cam0']
            lidar_data_token = sample_data_dict['LIDAR_TOP']

            cam_sd_record = nusc.get('sample_data', cam_data_token)
            cam_cs_record = nusc.get('calibrated_sensor', cam_sd_record['calibrated_sensor_token'])

            lidar_sd_record = nusc.get('sample_data', lidar_data_token)
            lidar_cs_record = nusc.get('calibrated_sensor', lidar_sd_record['calibrated_sensor_token'])

            ref_time = 1e-6 * lidar_sd_record['timestamp']

            ref_pose_rec = nusc.get('ego_pose', lidar_sd_record['ego_pose_token'])

            if lidar_sd_record['bestpos']['field.pos_type.type'] < 56.:
                # print("Localization not good enough, continuing:", lidar_sd_record['bestpos']['field.pos_type.type'])
                continue
            ...
            ref_cam_intrinsic = np.array(cam_cs_record['camera_intrinsic'])
            imsize = (cam_sd_record['width'], cam_sd_record['height'])

            # get paths
            ref_lidar_path = nusc.get_sample_data_path(lidar_data_token)
            ref_cam_path = nusc.get_sample_data_path(cam_data_token)
            lidar_path = Path(ref_lidar_path).relative_to(data_path).__str__()

            # check to see if number of fov points are larger than a threshold count
            points = np.fromfile(osp.join(data_path, str(lidar_path)), dtype=np.float32, count=-1).reshape([-1, 4])[:, :3]
            # imsize = info['imsize']
            pts_cam = transform_points(points, ref_to_cam)
            fov_flag = get_fov_flag(pts_cam, imsize, ref_cam_intrinsic)
            # points = points[fov_flag]
            if fov_flag.sum() < THRESHOLD_NUM_POINTS:
                print("Not enough points in fov, continuing:", fov_flag.sum(), str(lidar_path))
                continue
            else:
                info = {
                    ...
                    'lidar_token': lidar_data_token,
                    # 'token': lidar_data_token,  # NOTE THE CHANGE: now loading the lidar sample_data token, not the sample token
                    'sweeps': [],
    ...
    for det in det_annos:
        ...
        for k, box in enumerate(box_list):
            ...
            if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                if name in ['car', 'construction_vehicle', 'bus', 'truck', 'trailer', 'dynamic']:
                    attr = 'vehicle.moving'
